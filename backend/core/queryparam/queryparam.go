...
func BuildQueryParams(l logger.Logger, q url.Values) (*QueryParams, error) {
	if len(q) == 0 {
		return &QueryParams{
			Params: map[string][]QueryParam{},
			SortColumns: []SortColumn{
	...
	return &QueryParams{
		Params:      resolveQueryParamOps(qp),
...
func resolveQueryParamOps(qp url.Values) map[string][]QueryParam {
	result := map[string][]QueryParam{}

	for key, values := range qp {
		var op Operator
		split := strings.Split(key, ":") // assume colons in query params only appear to separate the query param from the operator
		if len(split) > 1 {
			key = split[0]
			op = Operator(split[1])
		}
