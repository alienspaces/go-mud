...
func TestCreateOne(t *testing.T) {
	...
	tests := []struct {
		name string
		rec  func(data harness.Data) *record.ActionMonsterObject
		err  bool
	}{
		{
			name: "Without ID",
			rec: func(data harness.Data) *record.ActionMonsterObject {
				return &record.ActionMonsterObject{
					ActionMonsterID:  data.ActionMonsterRecs[0].ID,
		...
		},
		{
			name: "With ID",
			rec: func(data harness.Data) *record.ActionMonsterObject {
				rec := &record.ActionMonsterObject{
					ActionMonsterID:  data.ActionMonsterRecs[0].ID,
					ObjectInstanceID: data.ObjectInstanceRecs[0].ID,
					Name:             data.ObjectRecs[0].Name,
					IsEquipped:       true,
	...
	for _, tc := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// repository
			r := h.Model.(*model.Model).ActionMonsterObjectRepository()
...
func TestGetOne(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name: "With ID",
			id: func() string {
				return h.Data.ActionMonsterObjectRecs[0].ID
			},
			err: false,
		},
		{
...
func TestUpdateOne(t *testing.T) {
	...
	tests := []struct {
		name string
		rec  func() *record.ActionMonsterObject
		err  bool
	}{
		{
			name: "With ID",
			rec: func() *record.ActionMonsterObject {
				rec := *h.Data.ActionMonsterObjectRecs[0]
				return &rec
			},
			err: false,
		},
