...
func TestCreateOne(t *testing.T) {
	...
	tests := []struct {
		name string
		rec  func(data harness.Data) *record.LocationObject
		err  bool
	}{
		{
			name: "Without ID",
	...
	for _, tc := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// repository
			r := h.Model.(*model.Model).LocationObjectRepository()
...
func TestUpdateOne(t *testing.T) {
	...
	tests := []struct {
		name string
		rec  func() *record.LocationObject
		err  bool
	}{
		{
			name: "With ID",
			rec: func() *record.LocationObject {
				return h.Data.LocationObjectRecs[0]
			},
			err: false,
		},
		{
			name: "Without ID",
			rec: func() *record.LocationObject {
				rec := h.Data.LocationObjectRecs[0]
				rec.ID = ""
				return rec
			},
			err: true,
