...
func TestCreateOne(t *testing.T) {
	...
	for _, tc := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// repository
			r := h.Model.(*model.Model).MonsterObjectRepository()
...
func TestGetOne(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name: "With ID",
			id: func() string {
				return h.Data.MonsterObjectRecs[0].ID
			},
			err: false,
		},
		{
...
func TestUpdateOne(t *testing.T) {
	...
	tests := []struct {
		name string
		rec  func() *record.MonsterObject
		err  bool
	}{
		{
			name: "With ID",
			rec: func() *record.MonsterObject {
				return h.Data.MonsterObjectRecs[0]
			},
			err: false,
		},
		{
			name: "Without ID",
			rec: func() *record.MonsterObject {
				rec := h.Data.MonsterObjectRecs[0]
				rec.ID = ""
				return rec
			},
			err: true,
